...
if ( ! function_exists( 'goo_start_cleanup' ) ) :
function goo_start_cleanup() {

	// Launching operation cleanup.
	add_action( 'init', 'goo_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'goo_remove_rss_version' );

	// Remove pesky injected css for recent comments widget.
	add_filter( 'wp_head', 'goo_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in the head.
	add_action( 'wp_head', 'goo_remove_recent_comments_style', 1 );

	// Clean up gallery output in wp.
	add_filter( 'goo_gallery_style', 'goo_gallery_style' );

}
add_action( 'after_setup_theme','goo_start_cleanup' );
endif;
/**
 * Clean up head.+
 * ----------------------------------------------------------------------------
...
if ( ! function_exists( 'goo_cleanup_head' ) ) :
function goo_cleanup_head() {
	...
	add_filter( 'style_loader_src', 'goo_remove_wp_ver_css_js', 9999 );

	// Remove WP version from scripts.
	add_filter( 'script_loader_src', 'goo_remove_wp_ver_css_js', 9999 );

}
endif;

// Remove WP version from RSS.
if ( ! function_exists( 'goo_remove_rss_version' ) ) :
function goo_remove_rss_version() { return ''; }
endif;

if ( ! function_exists( 'goo_remove_wp_ver_css_js' ) ) :

// Remove WP version from scripts.
function goo_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) ) {
...
if ( ! function_exists( 'goo_remove_wp_widget_recent_comments_style' ) ) :
function goo_remove_wp_widget_recent_comments_style() {
...
if ( ! function_exists( 'goo_remove_recent_comments_style' ) ) :
...
if ( ! function_exists( 'goo_gallery_style' ) ) :
function goo_gallery_style( $css ) {
...
if ( ! class_exists( 'goo_img_rebuilder' ) ) :
	class goo_img_rebuilder {
	...
	$goo_img_rebuilder = new goo_img_rebuilder;
...
add_action('woocommerce_before_main_content', 'goo_before_content', 10);
remove_action( 'woocommerce_after_main_content', 'woocommerce_output_content_wrapper_end', 10);
add_action('woocommerce_after_main_content', 'goo_after_content', 10);
?>
