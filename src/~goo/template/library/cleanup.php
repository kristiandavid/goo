...
if ( ! function_exists( 'breeyn_start_cleanup' ) ) :
function breeyn_start_cleanup() {

	// Launching operation cleanup.
	add_action( 'init', 'breeyn_cleanup_head' );

	// Remove WP version from RSS.
	add_filter( 'the_generator', 'breeyn_remove_rss_version' );

	// Remove pesky injected css for recent comments widget.
	add_filter( 'wp_head', 'breeyn_remove_wp_widget_recent_comments_style', 1 );

	// Clean up comment styles in the head.
	add_action( 'wp_head', 'breeyn_remove_recent_comments_style', 1 );

	// Clean up gallery output in wp.
	add_filter( 'breeyn_gallery_style', 'breeyn_gallery_style' );

}
add_action( 'after_setup_theme','breeyn_start_cleanup' );
endif;
/**
 * Clean up head.+
 * ----------------------------------------------------------------------------
...
if ( ! function_exists( 'breeyn_cleanup_head' ) ) :
function breeyn_cleanup_head() {
	...
	add_filter( 'style_loader_src', 'breeyn_remove_wp_ver_css_js', 9999 );

	// Remove WP version from scripts.
	add_filter( 'script_loader_src', 'breeyn_remove_wp_ver_css_js', 9999 );

}
endif;

// Remove WP version from RSS.
if ( ! function_exists( 'breeyn_remove_rss_version' ) ) :
function breeyn_remove_rss_version() { return ''; }
endif;

if ( ! function_exists( 'breeyn_remove_wp_ver_css_js' ) ) :

// Remove WP version from scripts.
function breeyn_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) ) {
...
if ( ! function_exists( 'breeyn_remove_wp_widget_recent_comments_style' ) ) :
function breeyn_remove_wp_widget_recent_comments_style() {
...
if ( ! function_exists( 'breeyn_remove_recent_comments_style' ) ) :
function breeyn_remove_recent_comments_style() {
...
if ( ! function_exists( 'breeyn_gallery_style' ) ) :
function breeyn_gallery_style( $css ) {
...
if ( ! class_exists( 'breeyn_img_rebuilder' ) ) :
	class breeyn_img_rebuilder {
	...
	$breeyn_img_rebuilder = new breeyn_img_rebuilder;
...
add_action('woocommerce_before_main_content', 'breeyn_before_content', 10);
remove_action( 'woocommerce_after_main_content', 'woocommerce_output_content_wrapper_end', 10);
add_action('woocommerce_after_main_content', 'breeyn_after_content', 10);
?>
